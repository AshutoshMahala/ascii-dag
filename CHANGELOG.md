...
- **Custom node painters (render engine)**: a node's reserved `width × height` area (from `add_node_with_size`) is now actually filled at render time. `NodeStyle` gains `paint: NodePaint` — `Simple` (`[label]` on the top row; the default, byte-identical to 0.9), `Boxed` (a light-stroke box spanning the full area with the label inside; falls back to `Simple` below 3 rows or 2 columns) or `Custom(fn)`: a plain `fn(&mut NodeRegion, NodePaintCtx)` that draws through a clipped, node-local writer — coordinates are node-relative and out-of-region writes are silent no-ops, so painters cannot touch neighboring cells and need no `unsafe`. `NodePaintCtx` (`#[non_exhaustive]`, engine-created) carries the node's id, label, dimensions, the active charset (painter text passes through untranslated — charset-faithful painters pick their glyphs from it), and `visible_rows` — the node-local row range visible in the band being composited, so tall painters can skip clipped content instead of re-drawing everything per band. Hit-testing now treats a node's full reserved rectangle as the node. A node spanning multiple bands has its painter replayed per band; drawing the same content each call is the painter's contract (see `NodeRegion` docs — canvas isolation is per logical cell; wide/control characters remain the caller's concern, as in labels). See `examples/node_painting.rs`.
...
- **`GraphError` and the render vocabulary enums are now `#[non_exhaustive]`** (`Charset`, `ColorMode`, `LineWeight`, `MarkerShape`, `NodeBorder`, `SubgraphBorder`, `LabelPosition`, `LabelPlacement`, `HitResult`): exhaustive `match`es need a `_` arm once, and future variants stop being breaking changes.
