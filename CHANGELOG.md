...
- **`hit_test` moved onto `Scene`; `RenderPlan` is no longer public.** The 0.10 introspection surface (`ir.render_plan(&options)` / `ir.render_plan_in(...)` / `ir.hit_test(&plan, x, y)`) is replaced by `planner.plan(&ir, &options.plan)` + `scene.hit_test(x, y)` — the scene carries its layout, so hit-testing can no longer be paired with the wrong one. `HitResult` gains a `Dummy { edge, level }` variant: a shown routing waypoint reports its SEMANTIC identity — the owning edge's input index plus the level, the same pair `NodeView::dummy_of` carries — instead of leaking a synthetic, backend-specific node id as `Node(id)` (0.10 behavior). All other variants and semantics are unchanged.
...
- **`LayoutIR::y_index`, `LayoutIR::items_at_line`, and `LineOccupancy`** → `ScenePlanner::new().plan(&ir, &options.plan)` + `scene.hit_test(x, y)`.
