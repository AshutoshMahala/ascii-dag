...
- **`hit_test` moved onto `Scene`; `RenderPlan` is no longer public.** The 0.10 introspection surface (`ir.render_plan(&options)` / `ir.render_plan_in(...)` / `ir.hit_test(&plan, x, y)`) is replaced by `planner.plan(&ir, &options.plan)` + `scene.hit_test(x, y)` — the scene carries its layout, so hit-testing can no longer be paired with the wrong one. `HitResult` and its semantics are unchanged.
