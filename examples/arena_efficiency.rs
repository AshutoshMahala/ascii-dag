...
fn test_alignment_waste() {
    ...
    println!(
        "  Alignment waste: {} bytes ({:.1}%)",
        wasted,
        wasted as f64 / offset as f64 * 100.0
    );
    println!();
}
...
fn analyze_over_allocation() {
    ...
    for (name, nodes, edges, actual_levels, actual_dummies) in cases {
        ...
        let allocated_temps = nodes * 8 +                           // node_levels
        ...
        let needed_temps = nodes * 8 +                           // node_levels
