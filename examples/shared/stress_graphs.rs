// Clustered stress graphs — five tiers of increasing subgraph depth and width.
//
// SINGLE SOURCE OF TRUTH, same pattern as hero_graph.rs: examples/subgraph_stress.rs
// and the layout-quality scorer in src/render/engine/parity.rs both pull this in
// via include!. These are the corpus's only graphs with deep nesting AND
// cross-cluster edges, which is what routing-quality work has to be measured on.
//
// Tier 1: 12 nodes,  4 subgraphs, depth 1
// Tier 2: 20 nodes,  8 subgraphs, depth 2
// Tier 3: 30 nodes, 12 subgraphs, depth 3
// Tier 4: enterprise scale
// Tier 5: megacorp scale (regions x layers)

// ── Tier 1: Microservices ────────────────────────────────────────────────
// 12 nodes, 4 subgraphs, depth 1 — the baseline "should look great"

fn tier1_microservices() -> ascii_dag::Graph<'static> {
...
fn tier2_platform() -> ascii_dag::Graph<'static> {
    let mut g = ascii_dag::Graph::new();
...
fn tier3_cloud() -> ascii_dag::Graph<'static> {
    let mut g = ascii_dag::Graph::new();
    let mut id = 0usize;
    let mut next = || {
        id += 1;
...
fn tier4_enterprise() -> ascii_dag::Graph<'static> {
    let mut g = ascii_dag::Graph::new();
    let mut id = 0usize;
    let mut next = || {
        id += 1;
...
fn tier5_megacorp() -> ascii_dag::Graph<'static> {
    let mut g = ascii_dag::Graph::new();
    let mut id = 0usize;
    let mut next = || {
        id += 1;
