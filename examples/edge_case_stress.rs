...
fn main() {
    ...
    test!("Empty DAG", {
        let dag: Graph = Graph::new();
        let _ = dag.render();
    });

    test!("Single node", {
        let dag = Graph::from_edges(&[(1, "alone")], &[]);
        let _ = dag.render();
    });

    test!("Single node with empty label", {
        let dag = Graph::from_edges(&[(1, "")], &[]);
        let _ = dag.render();
    });
    ...
    test!("Very long label (1000 chars)", {
        ...
        let dag = Graph::from_edges(&[(1, &long_label)], &[]);
        let _ = dag.render();
    });
    ...
    test!("Unicode: emoji", {
        let dag = Graph::from_edges(&[(1, "🔥 Fire"), (2, "💧 Water")], &[(1, 2)]);
        let _ = dag.render();
    });

    test!("Unicode: CJK characters", {
        let dag = Graph::from_edges(&[(1, "日本語"), (2, "中文")], &[(1, 2)]);
        let _ = dag.render();
    });

    test!("Unicode: RTL (Arabic)", {
        let dag = Graph::from_edges(&[(1, "مرحبا"), (2, "عالم")], &[(1, 2)]);
        let _ = dag.render();
    });
    ...
    test!("Whitespace only labels", {
        let dag = Graph::from_edges(&[(1, "   "), (2, "\t\t"), (3, "  \n  ")], &[(1, 2), (2, 3)]);
        let _ = dag.render();
    });

    test!("Null byte in label", {
        let dag = Graph::from_edges(&[(1, "before\0after")], &[]);
        let _ = dag.render();
    });

    test!("Control characters", {
        let dag = Graph::from_edges(&[(1, "\x01\x02\x03")], &[]);
        let _ = dag.render();
    });

    test!("Box drawing characters (conflict with rendering)", {
        let dag = Graph::from_edges(&[(1, "┌─┐│└┘├┤┬┴┼"), (2, "───────")], &[(1, 2)]);
        let _ = dag.render();
    });
    ...
    test!("Self-reference (node points to itself)", {
        let dag = Graph::from_edges(&[(1, "self")], &[(1, 1)]);
        let _ = dag.render();
    });

    test!("Circular dependency (A->B->A)", {
        let dag = Graph::from_edges(&[(1, "A"), (2, "B")], &[(1, 2), (2, 1)]);
        let _ = dag.render();
    });

    test!("Edge to non-existent node", {
        let dag = Graph::from_edges(&[(1, "exists")], &[(1, 999)]);
        let _ = dag.render();
    });

    test!("Edge from non-existent node", {
        let dag = Graph::from_edges(&[(1, "exists")], &[(999, 1)]);
        let _ = dag.render();
    });

    test!("Duplicate edges", {
        let dag = Graph::from_edges(&[(1, "A"), (2, "B")], &[(1, 2), (1, 2), (1, 2)]);
        let _ = dag.render();
    });

    test!("Duplicate node IDs", {
        let dag = Graph::from_edges(&[(1, "first"), (1, "second"), (1, "third")], &[]);
        let _ = dag.render();
    });
    ...
    test!("Many roots (disconnected nodes)", {
        ...
        let dag = Graph::from_edges(&nodes, &[]);
        let _ = dag.render();
    });
    ...
    test!("Large node ID (usize::MAX / 2)", {
        ...
        let dag = Graph::from_edges(&[(big_id, "big")], &[]);
        let _ = dag.render();
    });

    test!("Zero as node ID", {
        let dag = Graph::from_edges(&[(0, "zero")], &[]);
        let _ = dag.render();
    });
    ...
    test!("Horizontal mode", {
        let mut dag = Graph::from_edges(&[(1, "A"), (2, "B")], &[(1, 2)]);
    ...
    test!("Long labels + deep chain", {
        ...
        let dag = Graph::from_edges(&nodes_ref, &edges);
        let _ = dag.render();
    });
