...
fn main() {
    ...
    writeln!(f, "  D (id=4) should be level 3 (child of C and A, max+1=3)").unwrap();
    
    writeln!(f, "\nSkip edges:").unwrap();
    writeln!(f, "  A->D spans levels 0 to 3, so needs dummies at levels 1 and 2").unwrap();
    ...
    writeln!(f, "  Y should be level 2 (child of B via 2->5, but also child of X via 4->5)").unwrap();
    writeln!(f, "  Z should be level 3 (child of C via 3->6, but also child of Y via 5->6)").unwrap();
