...
fn main() {
    ...
    let tests: Vec<(&str, fn() -> Graph<'static>)> = vec![
        ("Tier 1 · Microservices (12 nodes, 4 subgraphs, depth 1)", tier1_microservices),
        ("Tier 2 · Platform (20 nodes, 8 subgraphs, depth 2)", tier2_platform),
        ("Tier 3 · Cloud Infra (30 nodes, 12 subgraphs, depth 3)", tier3_cloud),
        ("Tier 4 · Enterprise (50 nodes, 16 subgraphs, depth 3)", tier4_enterprise),
        ("Tier 5 · Megacorp (80 nodes, 24 subgraphs, depth 4)", tier5_megacorp),
    ];
...
fn render_csr(dag: &Graph, sgs: usize) {
    ...
    let csr = match dag.to_csr(&mut csr_arena) {
        Some(g) => g,
        None => {
            println!("  (CSR conversion failed — arena too small: {} KB)", csr_size / 1024);
            return;
        }
    };
    ...
    let ir = match csr.compute_layout_arena(&LayoutConfig::standard(), &mut temp_arena, &mut out_arena) {
        Ok(ir) => ir,
        Err(e) => {
    ...
    if let Ok(s) = std::str::from_utf8(&render_buf[..bytes]) {
        ...
        println!(
            ...
            sgs, max_width, lines, elapsed, (csr_size + size * 2) / 1024
        );
    }
}
...
fn tier2_platform() -> Graph<'static> {
    ...
    g.put_subgraphs(&[monitoring, logging]).inside(observability).unwrap();
...
fn tier3_cloud() -> Graph<'static> {
    ...
    // Ingress
    let lb = next(); g.add_node(lb, "ALB");
    let waf = next(); g.add_node(waf, "WAF");
    g.add_edge(lb, waf, None);

    // Region A — AZ1
    let a1_web = next(); g.add_node(a1_web, "Web-A1");
    let a1_app = next(); g.add_node(a1_app, "App-A1");
    let a1_db = next(); g.add_node(a1_db, "DB-A1");
    g.add_edge(waf, a1_web, None);
    g.add_edge(a1_web, a1_app, None);
    g.add_edge(a1_app, a1_db, None);

    // Region A — AZ2
    let a2_web = next(); g.add_node(a2_web, "Web-A2");
    let a2_app = next(); g.add_node(a2_app, "App-A2");
    let a2_db = next(); g.add_node(a2_db, "DB-A2");
    g.add_edge(waf, a2_web, None);
    g.add_edge(a2_web, a2_app, None);
    g.add_edge(a2_app, a2_db, None);

    // Region B — AZ1
    let b1_web = next(); g.add_node(b1_web, "Web-B1");
    let b1_app = next(); g.add_node(b1_app, "App-B1");
    let b1_cache = next(); g.add_node(b1_cache, "Redis-B1");
    let b1_db = next(); g.add_node(b1_db, "DB-B1");
    ...
    // Region B — AZ2
    let b2_web = next(); g.add_node(b2_web, "Web-B2");
    let b2_app = next(); g.add_node(b2_app, "App-B2");
    let b2_cache = next(); g.add_node(b2_cache, "Redis-B2");
    let b2_db = next(); g.add_node(b2_db, "DB-B2");
    ...
    // Shared services
    let mq = next(); g.add_node(mq, "RabbitMQ");
    let s3 = next(); g.add_node(s3, "S3-Bucket");
    let vault = next(); g.add_node(vault, "Vault");
    g.add_edge(a1_app, mq, None);
    g.add_edge(b1_app, mq, None);
    g.add_edge(mq, s3, None);

    // CI/CD
    let ci = next(); g.add_node(ci, "Jenkins");
    let cd = next(); g.add_node(cd, "ArgoCD");
    let registry = next(); g.add_node(registry, "ECR");
    ...
    // Monitoring
    let prom = next(); g.add_node(prom, "Prometheus");
    let grafana = next(); g.add_node(grafana, "Grafana");
    let loki = next(); g.add_node(loki, "Loki");
    ...
    g.put_nodes(&[b1_web, b1_app, b1_cache, b1_db]).inside(az_b1).unwrap();
    g.put_nodes(&[b2_web, b2_app, b2_cache, b2_db]).inside(az_b2).unwrap();
...
fn tier4_enterprise() -> Graph<'static> {
    ...
    let portal = next(); g.add_node(portal, "Portal");
    let mobile = next(); g.add_node(mobile, "MobileApp");

    // Auth domain
    let auth_gw = next(); g.add_node(auth_gw, "AuthGW");
    let oauth = next(); g.add_node(oauth, "OAuth2");
    let ldap = next(); g.add_node(ldap, "LDAP");
    let mfa = next(); g.add_node(mfa, "MFA");

    // Customer domain
    let cust_api = next(); g.add_node(cust_api, "CustAPI");
    let cust_svc = next(); g.add_node(cust_svc, "CustSvc");
    let cust_db = next(); g.add_node(cust_db, "CustDB");
    let cust_cache = next(); g.add_node(cust_cache, "CustCache");

    // Product domain
    let prod_api = next(); g.add_node(prod_api, "ProdAPI");
    let catalog = next(); g.add_node(catalog, "Catalog");
    let inventory = next(); g.add_node(inventory, "Inventory");
    let pricing = next(); g.add_node(pricing, "Pricing");
    let prod_db = next(); g.add_node(prod_db, "ProdDB");

    // Order domain
    let order_api = next(); g.add_node(order_api, "OrderAPI");
    let order_svc = next(); g.add_node(order_svc, "OrderSvc");
    let order_db = next(); g.add_node(order_db, "OrderDB");
    let cart = next(); g.add_node(cart, "CartSvc");
    let cart_db = next(); g.add_node(cart_db, "CartDB");

    // Payment domain
    let pay_gw = next(); g.add_node(pay_gw, "PayGW");
    let pay_proc = next(); g.add_node(pay_proc, "PayProc");
    let pay_db = next(); g.add_node(pay_db, "PayDB");
    let fraud = next(); g.add_node(fraud, "FraudDet");

    // Shipping domain
    let ship_api = next(); g.add_node(ship_api, "ShipAPI");
    let ship_svc = next(); g.add_node(ship_svc, "ShipSvc");
    let ship_db = next(); g.add_node(ship_db, "ShipDB");
    let tracking = next(); g.add_node(tracking, "Tracking");

    // Messaging backbone
    let kafka = next(); g.add_node(kafka, "Kafka");
    let schema_reg = next(); g.add_node(schema_reg, "SchemaReg");

    // Data Platform
    let spark = next(); g.add_node(spark, "Spark");
    let datalake = next(); g.add_node(datalake, "DataLake");
    let airflow = next(); g.add_node(airflow, "Airflow");
    let redshift = next(); g.add_node(redshift, "Redshift");

    // Observability
    let prom = next(); g.add_node(prom, "Prometheus");
    let grafana = next(); g.add_node(grafana, "Grafana");
    let jaeger = next(); g.add_node(jaeger, "Jaeger");
    let elk = next(); g.add_node(elk, "ELK");
    let pager = next(); g.add_node(pager, "PagerDuty");

    // DevOps
    let github = next(); g.add_node(github, "GitHub");
    let ci = next(); g.add_node(ci, "CI-Runner");
    let terraform = next(); g.add_node(terraform, "Terraform");
    let k8s = next(); g.add_node(k8s, "K8s");
    let helm = next(); g.add_node(helm, "Helm");

    // Notification
    let notif = next(); g.add_node(notif, "NotifSvc");
    let email = next(); g.add_node(email, "EmailGW");
    let sms = next(); g.add_node(sms, "SMS-GW");
    let push = next(); g.add_node(push, "PushSvc");
    ...
    g.put_nodes(&[auth_gw, oauth, ldap, mfa]).inside(sg_auth).unwrap();

    let sg_cust = g.add_subgraph("Customer");
    g.put_nodes(&[cust_api, cust_svc, cust_db, cust_cache]).inside(sg_cust).unwrap();

    let sg_catalog = g.add_subgraph("Catalog");
    g.put_nodes(&[catalog, pricing, prod_db]).inside(sg_catalog).unwrap();
    ...
    g.put_nodes(&[order_api, order_svc, order_db]).inside(sg_order).unwrap();
    g.put_subgraphs(&[sg_cart]).inside(sg_order).unwrap();

    let sg_pay = g.add_subgraph("Payments");
    g.put_nodes(&[pay_gw, pay_proc, pay_db, fraud]).inside(sg_pay).unwrap();

    let sg_ship = g.add_subgraph("Shipping");
    g.put_nodes(&[ship_api, ship_svc, ship_db, tracking]).inside(sg_ship).unwrap();
    ...
    g.put_nodes(&[spark, datalake, airflow, redshift]).inside(sg_dp).unwrap();

    let sg_mon = g.add_subgraph("Monitoring");
    g.put_nodes(&[prom, grafana, jaeger, elk, pager]).inside(sg_mon).unwrap();
    ...
    g.put_nodes(&[notif, email, sms, push]).inside(sg_notif).unwrap();

    let sg_devops = g.add_subgraph("DevOps");
    g.put_nodes(&[github, ci, terraform, k8s, helm]).inside(sg_devops).unwrap();
...
fn tier5_megacorp() -> Graph<'static> {
    ...
    // ── Global Entry ──
    let dns = next(); g.add_node(dns, "Route53");
    let cdn = next(); g.add_node(cdn, "CloudFront");
    let waf = next(); g.add_node(waf, "WAF");
    ...
    // ── US Region ──
    //   Frontend
    let us_web = next(); g.add_node(us_web, "US-Web");
    let us_mobile = next(); g.add_node(us_mobile, "US-Mobile");
    let us_ssr = next(); g.add_node(us_ssr, "US-SSR");
    g.add_edge(waf, us_web, None);
    g.add_edge(waf, us_mobile, None);
    g.add_edge(us_web, us_ssr, None);

    //   Auth micro
    let us_auth = next(); g.add_node(us_auth, "US-Auth");
    let us_token = next(); g.add_node(us_token, "US-Token");
    let us_mfa = next(); g.add_node(us_mfa, "US-MFA");
    ...
    //   Biz logic
    let us_order = next(); g.add_node(us_order, "US-Orders");
    let us_pay = next(); g.add_node(us_pay, "US-Pay");
    let us_inv = next(); g.add_node(us_inv, "US-Inv");
    let us_ship = next(); g.add_node(us_ship, "US-Ship");
    ...
    //   Databases
    let us_pg = next(); g.add_node(us_pg, "US-Postgres");
    let us_redis = next(); g.add_node(us_redis, "US-Redis");
    let us_s3 = next(); g.add_node(us_s3, "US-S3");
    ...
    let eu_web = next(); g.add_node(eu_web, "EU-Web");
    let eu_auth = next(); g.add_node(eu_auth, "EU-Auth");
    let eu_order = next(); g.add_node(eu_order, "EU-Orders");
    let eu_pay = next(); g.add_node(eu_pay, "EU-Pay");
    let eu_ship = next(); g.add_node(eu_ship, "EU-Ship");
    let eu_pg = next(); g.add_node(eu_pg, "EU-Postgres");
    let eu_redis = next(); g.add_node(eu_redis, "EU-Redis");
    ...
    // ── APAC Region ──
    let ap_web = next(); g.add_node(ap_web, "AP-Web");
    let ap_auth = next(); g.add_node(ap_auth, "AP-Auth");
    let ap_order = next(); g.add_node(ap_order, "AP-Orders");
    let ap_pg = next(); g.add_node(ap_pg, "AP-Postgres");
    ...
    // ── Messaging Layer ──
    let kafka1 = next(); g.add_node(kafka1, "Kafka-1");
    let kafka2 = next(); g.add_node(kafka2, "Kafka-2");
    let zk = next(); g.add_node(zk, "Zookeeper");
    let schema = next(); g.add_node(schema, "SchemaReg");
    ...
    // ── Data Platform ──
    let spark = next(); g.add_node(spark, "Spark");
    let flink = next(); g.add_node(flink, "Flink");
    let airflow = next(); g.add_node(airflow, "Airflow");
    let datalake = next(); g.add_node(datalake, "DataLake");
    let redshift = next(); g.add_node(redshift, "Redshift");
    let tableau = next(); g.add_node(tableau, "Tableau");
    ...
    // ── ML Platform ──
    let mlflow = next(); g.add_node(mlflow, "MLflow");
    let sagemaker = next(); g.add_node(sagemaker, "SageMaker");
    let model_reg = next(); g.add_node(model_reg, "ModelReg");
    let inference = next(); g.add_node(inference, "Inference");
    ...
    let jaeger = next(); g.add_node(jaeger, "Jaeger");
    let loki = next(); g.add_node(loki, "Loki");
    let cortex = next(); g.add_node(cortex, "Cortex");
    let pager = next(); g.add_node(pager, "PagerDuty");
    let opsgenie = next(); g.add_node(opsgenie, "OpsGenie");
    ...
    // ── Security ──
    let vault = next(); g.add_node(vault, "Vault");
    let cert_mgr = next(); g.add_node(cert_mgr, "CertMgr");
    let guard = next(); g.add_node(guard, "GuardDuty");
    let inspector = next(); g.add_node(inspector, "Inspector");
    ...
    // ── DevOps / Platform ──
    let github = next(); g.add_node(github, "GitHub");
    let ci = next(); g.add_node(ci, "Actions");
    let ecr = next(); g.add_node(ecr, "ECR");
    let argo = next(); g.add_node(argo, "ArgoCD");
    let tf = next(); g.add_node(tf, "Terraform");
    let k8s_us = next(); g.add_node(k8s_us, "EKS-US");
    let k8s_eu = next(); g.add_node(k8s_eu, "EKS-EU");
    let k8s_ap = next(); g.add_node(k8s_ap, "EKS-AP");
    g.add_edge(github, ci, None);
    ...
    // ── Notifications ──
    let sns = next(); g.add_node(sns, "SNS");
    let ses = next(); g.add_node(ses, "SES");
    let slack_hook = next(); g.add_node(slack_hook, "Slack");
    ...
    g.put_nodes(&[us_web, us_mobile, us_ssr]).inside(us_fe).unwrap();
    let us_au = g.add_subgraph("US-Auth");
    g.put_nodes(&[us_auth, us_token, us_mfa]).inside(us_au).unwrap();
    let us_biz = g.add_subgraph("US-Business");
    g.put_nodes(&[us_order, us_pay, us_inv, us_ship]).inside(us_biz).unwrap();
    let us_data = g.add_subgraph("US-Data");
    g.put_nodes(&[us_pg, us_redis, us_s3]).inside(us_data).unwrap();
    let region_us = g.add_subgraph("US-East");
    g.put_subgraphs(&[us_fe, us_au, us_biz, us_data]).inside(region_us).unwrap();

    // EU Region
    let eu_svc = g.add_subgraph("EU-Services");
    g.put_nodes(&[eu_web, eu_auth, eu_order, eu_pay, eu_ship]).inside(eu_svc).unwrap();
    ...
    g.put_nodes(&[ap_web, ap_auth, ap_order, ap_pg]).inside(region_ap).unwrap();

    // Messaging
    let sg_msg = g.add_subgraph("Event Bus");
    g.put_nodes(&[kafka1, kafka2, zk, schema]).inside(sg_msg).unwrap();
    ...
    g.put_subgraphs(&[sg_ingest, sg_store]).inside(sg_dp).unwrap();

    // ML Platform
    let sg_ml = g.add_subgraph("ML Platform");
    g.put_nodes(&[mlflow, sagemaker, model_reg, inference]).inside(sg_ml).unwrap();

    // Observability
    let sg_metrics = g.add_subgraph("Metrics");
    g.put_nodes(&[prom, cortex, grafana]).inside(sg_metrics).unwrap();
    ...
    let sg_obs = g.add_subgraph("Observability");
    g.put_subgraphs(&[sg_metrics, sg_tracing, sg_alert]).inside(sg_obs).unwrap();

    // Security
    let sg_sec = g.add_subgraph("Security");
    g.put_nodes(&[vault, cert_mgr, guard, inspector]).inside(sg_sec).unwrap();

    // DevOps
    let sg_cicd = g.add_subgraph("CI/CD");
    g.put_nodes(&[github, ci, ecr, argo]).inside(sg_cicd).unwrap();
    let sg_infra = g.add_subgraph("Infrastructure");
    g.put_nodes(&[tf, k8s_us, k8s_eu, k8s_ap]).inside(sg_infra).unwrap();
    let sg_devops = g.add_subgraph("Platform Eng");
    g.put_subgraphs(&[sg_cicd, sg_infra]).inside(sg_devops).unwrap();

    // Notifications
    let sg_notif = g.add_subgraph("Notifications");
    g.put_nodes(&[sns, ses, slack_hook]).inside(sg_notif).unwrap();
